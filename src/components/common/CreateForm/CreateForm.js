...
const CreateForm = (props) => {
    ...
    return (<form className={cn(styles.main, "w-11/12", "h-full", "flex", "flex-col", "text-3xl", "justify-evenly")}>
        ...
        <div className={cn(props.creatingQuestionType === "text" ? "" : styles.hide, "flex", "flex-col", "justify-between")}>
            ...
            <input value={question.value} onChange={e => question.onChange(e)} onBlur={e => question.onBlur(e)} id="question" name="question" placeholder="Введите вопрос" />
        </div>

        {(option1.isVisited && option1.isEmpty) && <div className={cn("text-red-800")}>{option1.isEmptyErrorMessage}</div>}
        <input value={option1.value} onChange={e => option1.onChange(e)} onBlur={e => option1.onBlur(e)} id="option1" name="option1" placeholder={'Вариант ответа номер 1'} />

        {(option2.isVisited && option2.isEmpty) && <div className={cn("text-red-800")}>{option2.isEmptyErrorMessage}</div>}
        <input value={option2.value} onChange={e => option2.onChange(e)} onBlur={e => option2.onBlur(e)} id="option2" name="option2" placeholder={'Вариант ответа номер 2'} />

        {(option3.isVisited && option3.isEmpty) && <div className={cn("text-red-800")}>{option3.isEmptyErrorMessage}</div>}
        <input value={option3.value} onChange={e => option3.onChange(e)} onBlur={e => option3.onBlur(e)} id="option3" name="option3" placeholder={'Вариант ответа номер 3'} />

        {(correctAnswer.isVisited && correctAnswer.isEmpty) && <div className={cn("text-red-800")}>{correctAnswer.isEmptyErrorMessage}</div>}
        {(correctAnswer.isVisited && correctAnswer.permissibleNumberValue) && <div className={cn("text-red-800")}>{correctAnswer.permissibleNumberValueErrorMessage}</div>}
        <input value={correctAnswer.value} onChange={e => correctAnswer.onChange(e)} onBlur={e => correctAnswer.onBlur(e)} id="correctAnswer" name="correctAnswer" placeholder={'Номер верного ответа'} />
